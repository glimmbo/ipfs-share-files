...
import ConnectedMapPage from './connected-map-page'

export const Page = (): React.JSX.Element => {
  const [t] = useTranslation()
  const currentPage = useCurrentPage()
  const heliaState = useHelia()
  const isControl = currentPage === 'control'
  const isDownload = currentPage === 'download'
  const pageTitle = isControl ? t('pageTitle.control') : isDownload ? t('pageTitle.download') : t('pageTitle.add')
  let content

  if (heliaState.error != null) {
    content = <BoxNotAvailable error={heliaState.error} />
  } else if (isControl) {
    content = <ConnectedMapPage />
  } else if (isDownload) {
    content = <BoxDownload />
  } else {
    content = <BoxAdd />
  ...
  return (
    <div data-id='Page'>
      <Helmet>
        <title>{t('pageTitle.ipfs')} | {pageTitle}</title>
      </Helmet>
      {isControl
        ? content
        : (
